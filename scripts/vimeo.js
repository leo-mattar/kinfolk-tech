...
function artifactStatus() {
  const items = document.querySelectorAll("[data-artifact-item]");
  const videos = document.querySelectorAll("[data-artifact-vimeo]");

  if (items.length === 0 || videos.length === 0) return;

  items.forEach(item => {
    item.addEventListener("click", () => {
      const targetValue = item.getAttribute("data-artifact-item");

      // Close all videos first
      videos.forEach(video => closeVideo(video));

      // Find the one matching video
      const matchedVideo = Array.from(videos).find(
        video => video.getAttribute("data-artifact-vimeo") === targetValue
      );

      if (matchedVideo) {
        matchedVideo.classList.add("is-open");

        const playButton = matchedVideo.querySelector(".vimeo-player__play");
        if (playButton) playButton.click();

        if (typeof lenis !== "undefined" && lenis.stop) lenis.stop();

        setupClose(matchedVideo);
      }
    });
  });

  function setupClose(video) {
    const closeBtn = video.querySelector(".c-vimeo-btn-close");
    if (closeBtn && !closeBtn.hasAttribute("data-close-attached")) {
      closeBtn.addEventListener("click", () => closeVideo(video));
      closeBtn.setAttribute("data-close-attached", "true");
    }

    const overlay = video.querySelector(".c-vimeo-overlay");
    if (overlay && !overlay.hasAttribute("data-close-attached")) {
      overlay.addEventListener("click", () => closeVideo(video));
      overlay.setAttribute("data-close-attached", "true");
    }

    if (!video.hasAttribute("data-esc-handler")) {
      const escHandler = e => {
        if (e.key === "Escape" && video.classList.contains("is-open")) {
          closeVideo(video);
        }
      };
      document.addEventListener("keydown", escHandler);
      video.setAttribute("data-esc-handler", "true");
      video._escHandler = escHandler;
    }
  }

  function closeVideo(video) {
    video.classList.remove("is-open");
    const pauseButton = video.querySelector(".vimeo-player__pause");
    if (pauseButton) pauseButton.click();
    if (typeof lenis !== "undefined" && lenis.start) lenis.start();

    if (video.hasAttribute("data-esc-handler")) {
      document.removeEventListener("keydown", video._escHandler);
      video.removeAttribute("data-esc-handler");
      delete video._escHandler;
    }
  }
}

function moreVideosHandler() {
  const el = document.querySelector("[data-stories-more]");
  if (!el) return;
