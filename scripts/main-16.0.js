...
function parallaxElements() {
  ...
  // Kill only ScrollTriggers for parallax elements
  parallaxEl.forEach(item => {
    ...
    gsap.set(item, { yPercent: 0, clearProps: "transform" });
  });
...
function homeTechPhoneAnimation() {
  const sections = document.querySelectorAll("[data-section-phone-animation]");

  if (!sections.length) return;

  sections.forEach(section => {
    const customX = section.getAttribute("data-x") || "30em";
    const items = section.querySelectorAll("[data-item-phone-animation]");
...
function videoScrollAnimation() {
  ...
  videosWrapper.forEach(video => {
    const mask = video.querySelector(".c-vimeo-mask");
...
function glitchIcons() {
    ...
    entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          startGlitchCycle(entry.target);
        } else {
          stopGlitchCycle(entry.target);
        }
      });
    },
    {
  ...
  icons.forEach(icon => {
    ...
    icon.glitchImages = images;
    icon.currentIndex = currentIndex;
    icon.hasCompletedCycle = false;

    observer.observe(icon);
  });

  function startGlitchCycle(icon) {
    if (icon.glitchInterval || icon.hasCompletedCycle) return;

    const images = icon.glitchImages;
    let currentIndex = icon.currentIndex;
    let cycleCount = 0;
    const totalImages = images.length;

    icon.glitchInterval = setInterval(() => {
      const currentImage = images[currentIndex];
      const nextIndex = (currentIndex + 1) % images.length;
      const nextImage = images[nextIndex];
      ...
      if (cycleCount >= 1) {
        clearInterval(icon.glitchInterval);
        icon.glitchInterval = null;
        icon.hasCompletedCycle = true;
      }
    }, 1250);
  }

  function stopGlitchCycle(icon) {
    if (icon.glitchInterval) {
      clearInterval(icon.glitchInterval);
      icon.glitchInterval = null;
    }
    icon.hasCompletedCycle = false;
...
//   const container = document.querySelector(".o-row.card-fx");
//   if (!container) return;
...
function animatedCards() {
  ...
  ScrollTrigger.create({
    ...
    start: () =>
      `top+=${
        (totalCards - 1 - (totalCards - 1)) * window.innerHeight -
        window.innerHeight
      } top`, // 1 viewport before first animation
    onEnter: () => {
      if (!hasRefreshed) {
        ScrollTrigger.refresh();
        hasRefreshed = true;
...
function splitTextLines() {
  document.fonts.ready.then(() => {
    let split = SplitText.create("[data-split-lines]", {
      type: "lines",
      autoSplit: true,
